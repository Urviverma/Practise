	...
	return editableDirectiveFactory({
		...
		render: function() {
			...
			**/
			...
			inputDatePicker.attr('init-date', this.attrs.eInitDate || false);
		...
		autosubmit: function () {
			// yq: made the bsdate to auto save
			var self = this;
			self.scope.$watch('$data', function (oldVal, newVal) {
			    if (oldVal != newVal) {
			        setTimeout(function () {
...
angular.module('xeditable').directive('editableDatepicker', ['editableDirectiveFactory', function (editableDirectiveFactory) {
	return editableDirectiveFactory({
	    directiveName: 'editableDatepicker',
	    inputTpl: '<input type="text" adit-datepicker ng-model="$data"/>',
	    render: function () {
	        this.scope.$data = moment(this.scope.$data).format('MM/DD/YYYY');
	        this.parent.render.call(this);
	        //this.inputEl.val(this.scope.$data);
	    },
	    autosubmit: function () {
	        // yq: made the bsdate to auto save
	        var self = this;
	        $(self.inputEl).datepicker()
...
angular.module('xeditable').directive('editableSelectize', ['editableDirectiveFactory', 'editableNgOptionsParser', function (editableDirectiveFactory, editableNgOptionsParser) {
    return editableDirectiveFactory({
        ...
        render: function () {
            ...
            var uiChoice = '<ui-select ng-model="customer.selected" multiple>' +
                                '<ui-select-match placeholder="Pick one...">{{$item.Name}}</ui-select-match>' +
                                '<ui-select-choices repeat="data in testdata">{{data.Name}}</ui-select-choices>' +
                           '</ui-select>';

            this.inputTpl = uiChoice;

            this.parent.render.call(this);

            var span = angular.element('<span></span>');
            span.attr('ng-model', this.inputEl.attr('ng-model'));

            this.inputEl.removeAttr('ng-model');
            this.inputEl.wrap(span);
        }
    })
}]);
...
angular.module('xeditable').factory('editableThemes', function() {
  var themes = {
    ...
    'bs3': {
      ...
      errorTpl:    '<div class="editable-error help-block" ng-show="$error" ng-bind="$error"></div>',
      buttonsTpl:  '<span class="editable-buttons"></span>',
      submitTpl:   '<button type="submit" id="inlineSubmit" class="btn btn-primary"><span></span></button>',
      cancelTpl:    '<button type="button" id="inlineCancel" class="btn btn-default" ng-click="$form.$cancel()">' +
                     '<span></span>'+
                   '</button>',
