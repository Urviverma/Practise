...
angular.module('xeditable').directive('editableDatepicker', ['editableDirectiveFactory', function (editableDirectiveFactory) {
	return editableDirectiveFactory({
		...
		inputTpl: '<input type="text" class="form-control input-sm" adit-datepicker ng-model="$data"/>',
		render: function () {
		...
		autosubmit: function () {
			// yq: made the bsdate to auto save
			var self = this;
			$(self.inputEl).datepicker().on('changeDate', function (e) {
            	setTimeout(function () {
            		self.scope.$apply(function () {
            			self.scope.$form.$submit();
            		});
...
angular.module('xeditable').factory('editableThemes', function() {
  var themes = {
    ...
    'bs2': {
      ...
      cancelTpl:   '<button type="button" id="inlineCancel" class="btn" ng-click="$form.$cancel()">' +
                      '<span></span>'+
                   '</button>'
